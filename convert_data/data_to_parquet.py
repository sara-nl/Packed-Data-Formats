...
def generate_parquet_data(
    ...
    if save_encoded:
        ...
        codec = (
            NdarrayCodec()
        )  # equivalent to  memfile = BytesIO(), np.save(memfile, value), bytearray(memfile.getvalue())
    ...
    MySchema = Unischema(
        "MySchema",
        [
            UnischemaField("image", np.uint8, image_shape, codec, False),
    ...
    with materialize_dataset(spark, output_url, MySchema, rowgroup_size_mb):
        rows_rdd = (
            sc.parallelize(range(num_samples))
            .map(lambda x: row_generator(x, dataset))
            .map(lambda x: dict_to_spark_row(MySchema, x))
        )

        spark.createDataFrame(rows_rdd, MySchema.as_spark_schema()).coalesce(
            num_partitions
...
def generate_cifar10_parquet(num_files, save_encoded):
    rowgroup_size_mb = 128
    output_url = "file:///home/{}/CompressionProject/data/cifar10/parquet/".format(
        os.getenv("USER")
    )
...
def generate_imagenet_parquet(num_files, save_encoded=False, resize=True):
    rowgroup_size_mb = 128
    output_url = (
        "file:///home/{}/CompressionProject/data/imagenet10k/parquet_encoded/".format(
            os.getenv("USER")
        )
    )
    ...
    dataset = ImageDataset(
        data_path, prefix="ILSVRC2012_val_", transform=transform_, offset_index=1
    )
    generate_parquet_data(
        dataset,
        output_url,
...
def generate_ffhq_parquet(num_files, save_encoded=False):
    ...
    output_url = "file:///scratch-shared/{}/ffhq_test/parquet_encoded/".format(
        os.getenv("USER")
    )
